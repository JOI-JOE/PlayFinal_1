            ...
            <div class="  w-full  sm:w-3/4 bg-white px-10 py-10">
                ...
                <div class="md:flex items-strech py-8 md:py-10 lg:py-8 border-t border-gray-50">
                    <div class="md:w-4/12 2xl:w-1/4 w-full">
                        <img src="{{ asset($item['img_thumbnail']) }}" alt="Black Leather Purse" class="h-full object-center object-cover md:block hidden" />
                        <img src="{{ asset($item['img_thumbnail']) }}" alt="Black Leather Purse" class="md:hidden w-full h-full object-center object-cover" />
                    </div>
                    <div class="md:pl-3 md:w-8/12 2xl:w-3/4 flex flex-col justify-center">
                        <div class="flex items-center justify-between w-full">
                            ...
                            <p class="py-2 px-4 border border-gray-200 mr-6 focus:outline-none ">
                                ...
                                <input type="text" value="{{ $item['quantity'] }}" class="w-12 text-center font-semibold text-xl">
                                @php
                        ...
                        <div class="flex items-center justify-between pt-5">
                            <div class="flex itemms-center">
                                ...
                                @endphp
                                <a href="{{ $url }}" onclick="return confirm('Are you sure?')" class="text-xs leading-3 underline text-red-500 pl-5 cursor-pointer">Remove</a>
                                </a>
                            </div>
            ...
            <div id="summary" class=" w-full   sm:w-1/4   md:w-1/2     px-8 py-10">
                ...
                <form action="{{ url('order/checkout') }} " method="POST">
                    <div class="mt-10">
                        ...
                        <input type="text" id="promo" placeholder="Enter your name" value="{{ $_SESSION['user']['name'] ?? null }}" name="user_name" class="p-2 text-sm w-full outline outline-2 outline-gray-300" />
                    </div>
                    <div class="my-10">
                        <label for="email" class="font-semibold inline-block mb-3 text-sm uppercase">
                            Email
                        </label>
                        <input type="email" id="email" placeholder="Enter your email" value="{{ $_SESSION['user']['email'] ?? null }}" name="user_email" class="p-2 text-sm w-full outline outline-2 outline-gray-300" />
                    </div>
                    <div class="my-10">
                        <label for="phone" class="font-semibold inline-block mb-3 text-sm uppercase">
                            Phone
                        </label>
                        <input type="tel" id="phone" placeholder="Enter your phone" value="{{ $_SESSION['user']['phone'] ?? null }}" name="user_phone" class="p-2 text-sm w-full outline outline-2 outline-gray-300" />
                    </div>
                    <div class="my-10">
                        <label for="address" class="font-semibold inline-block mb-3 text-sm uppercase">
                            Address
                        </label>
                        <input type="text" id="address" placeholder="Enter your address" value="{{ $_SESSION['user']['address'] ?? null }} " name="user_address" class="p-2 text-sm w-full outline outline-2 outline-gray-300" />
